...
@property (nonatomic, assign) BOOL showSingleNativeAdWhenLoaded; //flag to indicate that native ad should be shown as soon as it is loaded
...
- (void)viewDidLoad {
    ...
    self.configurePublishedIdsViewController = [[ConfigurePublishedIdsViewController alloc] initWithNibName:@"ConfigurePublisherIdsView" bundle:nil];
    self.configurePublishedIdsViewController.delegate = self;
    
    //read saved Publisher IDs
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSString* idForBanners = [defaults objectForKey:@"bannerIdKey"];
    NSString* idForInterstitials = [defaults objectForKey:@"interstitialIdKey"];
    NSString* idForNativeAds = [defaults objectForKey:@"nativeAdIdKey"];
    
    if(!idForBanners)
        idForBanners = @"ENTER_PUBLISHER_ID_HERE";  // your publisher IDs. NOTE: After you close the app, the Publisher IDs will be saved and used instead of values entered here.
    if(!idForInterstitials)                         // you can reset entered Publisher IDs by re-installing the application.
        idForInterstitials = @"ENTER_PUBLISHER_ID_HERE";
    if(!idForNativeAds)
        idForNativeAds = @"ENTER_PUBLISHER_ID_HERE";
    
    self.configurePublishedIdsViewController.idForBanners = idForBanners;
    self.configurePublishedIdsViewController.idForInterstitials = idForInterstitials;
    self.configurePublishedIdsViewController.idForNativeAds = idForNativeAds;
...
- (IBAction)showConfigDialog:(id)sender {
    [self presentViewController:self.configurePublishedIdsViewController animated:YES completion:nil];
}

-(void)configurePublisherIdsControllerIsDone {
    //save entered Publisher IDs
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    
    [defaults setValue:self.configurePublishedIdsViewController.idForBanners forKey:@"bannerIdKey"];
    [defaults setValue:self.configurePublishedIdsViewController.idForInterstitials forKey:@"interstitialIdKey"];
    [defaults setValue:self.configurePublishedIdsViewController.idForNativeAds forKey:@"nativeAdIdKey"];
...
- (IBAction)showSingleNativeAd:(id)sender {
    ...
    if(nativeAd) {
        ...
        self.showSingleNativeAdWhenLoaded = YES;
        [self requestNativeAdvert:self];
        NSLog(@"Native ad not ready, trying to load.");
    }
}
...
- (NSString *)publisherIdForMobFoxNativeAdController:(MobFoxNativeAdController *)controller {
    return self.configurePublishedIdsViewController.idForNativeAds;
}
...
- (void) nativeAdDidLoad:(MobFoxNativeAd *)ad {
    ...
    if(self.showSingleNativeAdWhenLoaded) {
        [self showSingleNativeAd:self];
    }
}
