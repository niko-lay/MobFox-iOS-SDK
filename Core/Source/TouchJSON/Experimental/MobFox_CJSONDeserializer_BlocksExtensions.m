...
#import "MobFox_CJSONDeserializer_BlocksExtensions.h"

#import "MobFox_CJSONScanner.h"

@implementation MobFox_CJSONDeserializer (CJSONDeserializer_BlocksExtensions)
...
- (void)deserializeAsDictionary:(NSData *)inData completionBlock:(void (^)(id result, NSError *error))block {
	...
	if (inData == NULL || [inData length] == 0) {
		noDataError = [NSError errorWithDomain:MobFox_kJSONDeserializerErrorDomain code:kJSONScannerErrorCode_NothingToScan userInfo:NULL];
...
- (void)deserializeAsArray:(NSData *)inData completionBlock:(void (^)(id result, NSError *error))block {
	...
	if (inData == NULL || [inData length] == 0) {
		nullInDataError = [NSError errorWithDomain:MobFox_kJSONDeserializerErrorDomain code:kJSONScannerErrorCode_NothingToScan userInfo:NULL];
